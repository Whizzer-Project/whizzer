...
public final class ResourcesUtils {
	 ...
	 * Field logger.
	 */
	private static Logger logger = LogManager.getLogger(ResourcesUtils.class.getName());
	...
	/**
	 * Field FILTER_TYPE_END. (value is ""not null"")
	 */
	static final String FILTER_TYPE_NOTNULL = "_notnull";

	/**
	 * Field ERROR_REASON_REFLECTION_ERROR. (value is ""Something bad happened when
	 * reflecting"")
	 */
	...
	/**
	 * Method optTimestamp. Converts a string value to a timestamp if the string is
	 * not empty. Returns null otherwise.
	 * 
	 * @param stringTime String
	 * @return Timestamp Returns null if the input string is null or empty.
	 * @throws ParseException
	 */
	public static Timestamp optTimestamp(String stringTime) throws ParseException {

		if ((null == stringTime) || (stringTime.isEmpty())) {
			return null;
		}

		return new Timestamp(new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").parse(stringTime).getTime());

	}

	/**
	 * Method optTime.Converts a string value to a time if the string is not empty.
	...
	public static Timestamp optTime(String stringTime) throws ParseException {
		...
		return new Timestamp(new SimpleDateFormat("hh:mm:ss").parse(stringTime).getTime());
	}
	 ...
	 * 
	 * @param stringTime String
	 * @return Timestamp Returns curent time if the input string is null or empty.
	 * @throws ParseException
	 */
	public static Timestamp getTimestamp(String stringTime) throws ParseException {

		Timestamp finalTimestamp = optTimestamp(stringTime);

		return (null == finalTimestamp) ? new Timestamp(System.currentTimeMillis()) : finalTimestamp;
	}
	 ...
	 * @param queryParamName String
	 * 
	 * @return String Null if the parameter with name queryParamName can not be
	 *         found.
	 */
	public static String getQueryParamValue(UriInfo uriInfo, String queryParamName) {
	 ...
	 * Method createLink.
	 * 
	 * @param baseObject JSONObject
	 * @param href       String
	 * @param rel        String
	 * @return JSONObject
	 * @throws JSONException
	 */
	public static JSONObject createLink(JSONObject baseObject, String href, String rel) throws JSONException {
		return baseObject.put("link", new JSONObject().put(LINK_HREF, href).put(LINK_REL, rel));
	}
	 ...
	 * @throws JSONException
	 */
	public static JSONObject createParentLink(JSONObject baseObject, String rel, UriInfo uriInfo) throws JSONException {
		...
		try {
			if (c.isAnnotationPresent(HtmlDoc.class)) {
				...
				if (!doc.delete().isEmpty()) {
					ret.append(String.format("<b>DELETE</b><br/>%s", doc.delete()));
				}
			}
		} catch (Exception e) {
			logger.error(String.format("Error retrieving description for class [%s]", c), e);
		}
	...
	public static String getClassDescription(String className) {
		try {
			...
			logger.error(String.format("Error retrieving description for class [%s]", className), e);
		}
	...
	public static TypedQuery<?> getTypedQuery(UriInfo uriInfo, EntityManager em, Class<?> entity, String filterName,
			String filterValue) {
		...
		if (params.containsKey(SORT_FIELD)) {
			try {
				...
				if (params.containsKey(SORT_ORDER) && params.getFirst(SORT_ORDER).equals("descending")) {
					query.orderBy(cb.desc(queryRoot.get(params.getFirst(SORT_FIELD))));
				} else {
					query.orderBy(cb.asc(queryRoot.get(params.getFirst(SORT_FIELD))));
				}
		...
		for (String filterFiled : params.keySet()) {
			if (filterFiled.startsWith(FILTER)) {
				if (filterFiled.endsWith(FILTER_TYPE_EXACT) || filterFiled.endsWith(FILTER_TYPE_CONTAINS)
						|| filterFiled.endsWith(FILTER_TYPE_END)) {
					filterEntityName = filterFiled.substring(filterFiled.indexOf("_") + 1,
				...
				try {
					...
					case FILTER_TYPE_EXACT:
						predicate = cb.and(predicate,
								cb.equal(queryRoot.get(filterEntityName), params.getFirst(filterFiled)));
						break;
					case FILTER_TYPE_CONTAINS:
						predicate = cb.and(predicate, cb.like(queryRoot.<String>get(filterEntityName),
					...
					case FILTER_TYPE_NOTNULL:
						...
						break;
					default:
						if (queryRoot.get(filterEntityName).getJavaType().toString().contains("java.sql.Timestamp")) {

							if (filterFiled.endsWith(FILTER_TYPE_END)) {
								predicate = cb.and(predicate,
										cb.lessThanOrEqualTo(queryRoot.<Timestamp>get(filterEntityName),
												getTimestamp(params.getFirst(filterFiled))));
							} else {
								predicate = cb.and(predicate,
										cb.greaterThanOrEqualTo(queryRoot.<Timestamp>get(filterEntityName),
												getTimestamp(params.getFirst(filterFiled))));
							}
						} else {
							predicate = cb.and(predicate, cb.like(queryRoot.<String>get(filterEntityName),
					...
					/*
					 * if (filterFiled.endsWith(FILTER_TYPE_EXACT)) { predicate = cb.and(predicate,
					 * cb.equal( queryRoot.get(filterEntityName), params.getFirst(filterFiled))); }
					 * else { if (filterFiled.endsWith(FILTER_TYPE_CONTAINS)) { predicate =
					 * cb.and(predicate, cb.like( queryRoot.<String> get(filterEntityName), "%" +
					 * params.getFirst(filterFiled) + "%")); } else { if
					 * (queryRoot.get(filterEntityName).getJavaType().toString()
					 * .contains("java.sql.Timestamp")) {
					 * 
					 * if (filterFiled.endsWith(FILTER_TYPE_END)) { predicate = cb .and(predicate,
					 * cb.lessThanOrEqualTo( queryRoot .<Timestamp> get(filterEntityName),
					 * getTimestamp(params .getFirst(filterFiled)))); } else { predicate = cb
					 * .and(predicate, cb.greaterThanOrEqualTo( queryRoot .<Timestamp>
					 * get(filterEntityName), getTimestamp(params .getFirst(filterFiled)))); } }
					 * else { predicate = cb .and(predicate, cb.like(queryRoot .<String>
					 * get(filterEntityName), params.getFirst(filterFiled) + "%"));
					 * 
					 * } } }
					 */
		...
		if (null != filterName) {
			predicate = cb.and(predicate, cb.equal(queryRoot.get(filterName), filterValue));
		}
		...
		for (String filterFiled : params.keySet()) {
			if (filterFiled.startsWith(FILTER)) {
				if (filterFiled.endsWith(FILTER_TYPE_EXACT)
						...
						|| filterFiled.endsWith(FILTER_TYPE_END)
						|| filterFiled.endsWith(FILTER_TYPE_NOTNULL)) {
				...
				try {
					...
					if (filterFiled.endsWith(FILTER_TYPE_EXACT)) {
						...
						if (filterFiled.endsWith(FILTER_TYPE_CONTAINS)) {
							...
							if (filterFiled.endsWith(FILTER_TYPE_NOTNULL)) {
								predicate = cb.and(predicate, cb.isNotNull(
										queryRoot.<String> get(filterEntityName)));
							} else {
	...
	public static String getColumnFromEntity(Class entity, String columnName) {
		if (!entity.isAnnotationPresent(Entity.class))
			throw new UnsupportedOperationException("Class " + entity.getName() + " is not an entity.");
	...
	public static Object getValueForAnnotation(Object o, Class<? extends Annotation> annotation) {
		Object r = null;
		boolean found = false;
		...
		for (Field field : o.getClass().getDeclaredFields())
			if (field.isAnnotationPresent(annotation)) {
				found = true;
				try {
					r = FieldUtils.readField(field, o, true);
		...
		if (!found)
			throw new RuntimeException(String.format("Entity %s - %s annotation missing", o.getClass().getName(),
