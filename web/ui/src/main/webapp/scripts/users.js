...
function selectLine(tr, username, state) {
	...
	if (typeof state !== 'undefined'){
		...
		$('table').find('select').prop('disabled', !state)
		$('table').find('select').next().find('button').prop('disabled', !state)
		if (!state)
			$('select').next().find('button').removeClass('disabled')
		else
			$('select').next().find('button').addClass('disabled')
	}
}
...
function resetTenantSelected(){
	$.each($('ul').find("input[type='checkbox']"), function(item, element){
...
function getUserRoles(username) {
	$.ajax({
		...
		success: function(data) {
			...
			$.each($.parseJSON(json.roles), function(i, item) {
			...
			resetTenantSelected()
			$('select').next().find('button').removeClass('disabled')
			$.each($.parseJSON(json.entity), function(i, item) {
				var roleid = item.roleId
				var value = item.entityId
				
				var slc = $("select[data-roleid='"+roleid+"']");
				slc.next().find("input[type='checkbox']").each(function(){
	...
	$("button.saveUserRole").each(function(){
		...
		$(this).click(function(){
			...
			$.each($('table').find('select'), function(){
				...
				var entity = $(this).next().find('button').attr('title')
				if (!entity.includes("None selected"))
				jsonEntity.push({
					'roleid': id,
					'entity': entity
				})
...
//			    },
				data: {
					"roles":JSON.stringify(jsonRoles),
					"entity":JSON.stringify(jsonEntity)
				},
