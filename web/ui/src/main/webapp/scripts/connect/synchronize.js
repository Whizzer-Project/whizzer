$(function() {
	...
	function getLocalAccounts(bic) {
		...
		$.ajax({
			...
			success : function(data) {
				window.accounts = JSON.parse(data);
				addItems($("#accountsIntern"), window.accounts.internal,"intern");
				addItems($('#accountsExtern'), window.accounts.external,"extern");
			},
			error : function(err) {
				console.log(err);
			},
	...
	function addItems(addToElement, getValue, name) {
		...
		$.each(getValue,function(item, value) {
			var balance = ''
			if (value.checked == 'true' && value.balance){
