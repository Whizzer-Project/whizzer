...
//	}
	...
	public ResponseEntity<String> insertListTemplateDetailed(List<TemplateDetailed> listTemplateDetailed) {
		URI uri = UriBuilder.fromUri(getBaseUrl()).path("" + listTemplateDetailed.get(0).getTxtemplate().getId()).path("fields").build();
		return post(uri, listTemplateDetailed.toArray(new TemplateDetailed[0]));
	}
