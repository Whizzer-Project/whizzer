...
public abstract class ApiDao<T> {
	...
	public T get(String id) {
		URI url = UriBuilder.fromUri(getBaseUrl()).path(id).path("consents").path(id).build();	
...
//	}
	...
	public PagedCollection<T> getPage(int page, LinkedHashMap<String, List<String>> params) {
...
//	}
	...
	public PagedCollection<T> getPage(URI uri) {
		return getPage(uri, new LinkedHashMap<>());
	}
	...
	public T[] getList(String id) {
		URI url = UriBuilder.fromUri(getBaseUrl()).path(id).path("consents").build();
	...
	public ResponseEntity<String> put(String id, T entity) {
		URI url = UriBuilder.fromUri(getBaseUrl()).path("consents").path(id).build();
